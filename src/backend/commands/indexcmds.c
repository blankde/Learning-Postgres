			...
			bool concurrent)
{
	...
	/* if not mltree, @btreeRelationId is invalid */
	btreeRelationId = InvalidOid;
	if(accessMethodId == MLTREE_AM_OID){
		char indexRelationName2[100];
		Datum		reloptions2;
		Oid		   *collationObjectId2;
		Oid		   *classObjectId2;
		int16	   *coloptions2;
		reloptions2 = transformRelOptions((Datum) 0, options, NULL, NULL, false, false);

		(void) index_reloptions(amoptions, reloptions2, true);

		collationObjectId2 = (Oid *) palloc(numberOfAttributes * sizeof(Oid));
		classObjectId2 = (Oid *) palloc(numberOfAttributes * sizeof(Oid));
		coloptions2 = (int16 *) palloc(numberOfAttributes * sizeof(int16));
		ComputeIndexAttrs(indexInfo, collationObjectId2, classObjectId2,
						  coloptions2, attributeList,
						  exclusionOpNames, relationId,
						  "btree", BTREE_AM_OID,
						  true, isconstraint);
		strcpy(indexRelationName2,indexRelationName);
		btreeRelationId = index_create3(rel, strcat(indexRelationName2,"_btree"), indexRelationId,
					 indexInfo, indexColNames,
					 BTREE_AM_OID, tablespaceId,
					 collationObjectId2, classObjectId2,
					 coloptions2, reloptions2, primary,
	 ...
	 */
	indexRelationId =
		index_create2(rel, indexRelationName, indexRelationId,btreeRelationId,
					 indexInfo, indexColNames,
