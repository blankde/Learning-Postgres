 ...
 *		heapgettup_pagemode - fetch next heap tuple in page-at-a-time mode
 *
 *		Same API as heapgettup, but only retrive one page
 * ----------------
 */
static void
heapgettup_onepage(HeapScanDesc scan,
					ScanDirection dir,
					int nkeys,
					ScanKey key)
{
...
HeapTuple
heap_getpagenext(HeapScanDesc scan, ScanDirection direction)
{
	/* Note: no locking manipulations needed */

	HEAPDEBUG_1;				/* heap_getnext( info ) */

	heapgettup_onepage(scan, direction,
						scan->rs_nkeys, scan->rs_key);
