 ...
 * allow_system_table_mods: allow tablindexRelationNamee to be a system catalog
			 ...
			 bool concurrent)
{
	...
	indexRelation->btreeIndexOid = InvalidOid;
 ...
 * index_create
 *
 * this is same as index_create() expect we added one args Oid btreeRelationI for create mltree
 */
Oid
			 ...
			 bool concurrent)
{
	...
	indexRelation->btreeIndexOid = InvalidOid;
	/* update pg_mlbtree */
	if(btreeRelationId != InvalidOid){
		Relation pg_mlbtree;
		pg_mlbtree = heap_open(MLBTREERelationId, RowExclusiveLock);
		InsertPgMlBtreeTuple(pg_mlbtree,indexRelationId,btreeRelationId);
		index_close(pg_mlbtree, NoLock);
		indexRelation->btreeIndexOid = btreeRelationId;
	}
...
/* this is for mlbtree */
Oid
index_create3(Relation heapRelation,
			 const char *indexRelationName,
			 Oid indexRelationId,
			 IndexInfo *indexInfo,
			 List *indexColNames,
...
/*			if(ismltree){
				blkno = root_blkno;
				pageinfo[blkno] = (blkno == 0) ? PageGetMaxOffsetNumber(page):pageinfo[blkno-1]+PageGetMaxOffsetNumber(page);
			}*/
		}
